...
view.setActiveScreen = (screenName) => {
  switch (screenName) {
    case "homeScreen":
      ...
      const searchingForm = document.getElementById(
        "input-searching-word-form"
      );
...
view.showDictionary = (listWord) => {
  ...
  if (listWord.length === 0) {
    document.querySelector(".center").innerHTML = `
            <center>
                <img src="../images/no_result2.webp" alt="" />
            </center>
        `;
    // const noResult = document.createElement('div');
    // noResult.className = "no-result";
    // noResult.innerHTML = "No result.";
    // document.querySelector('.aside-left').appendChild(noResult);
...
view.addFunctionForm = (nameFunctionForm) => {
  ...
  if (nameFunctionForm === "addWordForm") {
    ...
    if (isFirstRenderFunction === true) {
      ...
      document
        ...
        .classList.remove("currret-function");
      document.querySelector("#add-word").classList.add("currret-function");
    }
  }
  if (nameFunctionForm === "deleteWordForm") {
    ...
    deleteWordForm.addEventListener("submit", (event) => {
      event.preventDefault();
      const deleteWordData = deleteWordForm.deleteWordTarget.value
        .trim()
        .toLowerCase();
